...
def get_loaders(train_batch_size=25, test_batch_size=100):

    train_loader = torch.utils.data.DataLoader(
        torchvision.datasets.ImageFolder(
            './data/imagenet-mini/train',
...
def disable_gradients(module):
    if isinstance(module, nn.Sequential) or isinstance(module, models.resnet.Bottleneck):
...
train_loader, test_loader = get_loaders(train_batch_size=25)
...
plt.savefig('ResNet50-ImageNet-Trace_per_Layer')
...
plt.xlabel('Size (MB)')
plt.grid(True, which='both')
plt.savefig('ResNet50-ImageNet-Perturbation_vs_Size')
